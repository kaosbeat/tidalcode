...
let vsynthlist = [null, "cubes", "rects"]
let vsynths = {	"cubes": 
					{	
						"init": initCubes, 
						"seed": cubeSeed, 
...
let activesynths = {2 : "cubes"}


function initMidi(p5, midiobject){
	console.log("init midi. replacing noteOn. controlchange callbacks") 
	midiobject.noteOn = (note) => {
		// print(note)
		p5vars.hydracount+=1;
		switch (note.channel){
			case 16 : // control params
				if (note.velocity == 0) {
					delete activesynths[note.number]
				} else {
					activesynths[note.number] = vsynthlist[note.velocity]
				}
			// if (note.number != 0) {
			...
			case 6: // images
				if (note.number == 60) {
					updateImages(p5)
				} else if (note.number == 61) {
					updateImages(p5)
				}
				else {
					randomImage(p5)
				}
				// console.log(p5vars.vid)
				break;
			...
			case 7: // VIDEO
				...
				// console.log(p5.vid)
				let vidnumber = (note.number )%( viddataset.length)	
				console.log("vidnuimber")
				console.log(vidnumber	)
				// p5vars.vid.hide()
				p5.vid =  viddataset[vidnumber];
				p5.vid.show()
				p5.vid.volume(0);
				p5.vid.play();
				p5.vid.time(0);
	...
	midiobject.controlChange = (cc) => {
			...
			switch (index) {
				...
				case 10: 
					if (value ==  1) {
						...
						// }
						loginfo("cc10")
						// loginfo("code layer " + p5vars.coderoute)
					}
					break; // mix mode 

				case 11:  // tape channel 1  		
					console.log(value*127)				
					if (value == 1) { // code only
						

						p5vars.coderoute += 1
						if (p5vars.coderoute > 3) {p5vars.coderoute = 0}
						if (p5vars.coderoute == 0) {
							p5vars.codeblendpre = 0;
							p5vars.codeblendpost = 0;
						}
						if (p5vars.coderoute == 1) {
							p5vars.codeblendpre = 0.5;
							p5vars.codeblendpost = 0;
						}
						if (p5vars.coderoute == 2) {
							p5vars.codeblendpre = 0;
							p5vars.codeblendpost = 1;
						}
						if (p5vars.coderoute == 3) {
							p5vars.codeblendpre = 0.5;
							p5vars.codeblendpost = 1;
						}
						...
						loginfo("adding code route" + p5vars.coderoute) 

					}  else if (value*127 == 2) {
						console.log("settting stuiff")
						p5vars.codeblendpre = 0;
						p5vars.codeblendpost = 0;
						console.log(p5vars)
					}  else if (value*127 == 3) {
						console.log("setting p5vars 3")
						p5vars.codeblendpre = 0.5;
						p5vars.codeblendpost = 0;
						console.log(p5vars)
					}  else if (value*127 == 4) {
						console.log("setting p5vars 4")
						p5vars.codeblendpre = 0;
						p5vars.codeblendpost = 1;
						console.log(p5vars)
					} else if (value*127 == 5) {
						console.log("setting p5vars 5")
						p5vars.codeblendpre = 0.5;
						p5vars.codeblendpost = 1;
						console.log(p5vars)
					}
				...
				case 12:  // tape channel 2
					if (value == 1) { //WEBCAM
						p5vars.camroute += 1
						if (p5vars.camroute > 3) {p5vars.camroute = 0}
						if (p5vars.camroute == 0) {
							p5vars.camblendpre = 0;
							p5vars.camblendpost = 0;
						}
						if (p5vars.camroute == 1) {
							p5vars.camblendpre = 0.5;
							p5vars.camblendpost = 0;
						}
						if (p5vars.camroute == 2) {
							p5vars.camblendpre = 0;
							p5vars.camblendpost = 1;
						}
						if (p5vars.camroute == 3) {
							p5vars.camblendpre = 0.5;
							p5vars.camblendpost = 1;
						}
						loginfo("adding webcam route" + p5vars.camroute) 
					} else if (value*127 == 2) {
						p5vars.camblendpre = 0;
						p5vars.camblendpost = 0;
					}  else if (value*127 == 3) {
				...
				case 13: // tape channel 3
					if (value == 1) {  //P5
						p5vars.p5route += 1
						if (p5vars.p5route > 3) {p5vars.p5route = 0}
						if (p5vars.p5route == 0) {
							p5vars.p5blendpre = 0;
							p5vars.p5blendpost = 0;
						}
						if (p5vars.p5route == 1) {
							p5vars.p5blendpre = 0.5;
							p5vars.p5blendpost = 0;
						}
						if (p5vars.p5route == 2) {
							p5vars.p5blendpre = 0;
							p5vars.p5blendpost = 1;
						}
						if (p5vars.p5route == 3) {
							p5vars.p5blendpre = 0.5;
							p5vars.p5blendpost = 1;
						}
						loginfo("adding dataP5 route" + p5vars.p5route) 
					} else if (value*127 == 2) {
						p5vars.p5blendpre = 0;
						p5vars.p5blendpost = 0;
					}  else if (value*127 == 3) {
				...
				case 14: // tape channel 4
					if (value == 1) {
						...
						// else { p5vars.s3 = true;  }
						p5vars.extraroute += 1
						if (p5vars.extraroute > 3) {p5vars.extraroute = 0}
						if (p5vars.extraroute == 0) {
							p5vars.extrablendpre = 0;
							p5vars.extrablendpost = 0;
						}
						if (p5vars.extraroute == 1) {
							p5vars.extrablendpre = 0.5;
							p5vars.extrablendpost = 0;
						}
						if (p5vars.extraroute == 2) {
							p5vars.extrablendpre = 0;
							p5vars.extrablendpost = 1;
						}
						if (p5vars.extraroute == 3) {
							p5vars.extrablendpre = 0.5;
							p5vars.extrablendpost = 1;
						}
						loginfo("adding extra layer" + p5vars.extraroute )
					} else if (value*127 == 2) {
						p5vars.extrablendpre = 0;
						p5vars.extrablendpost = 0;
					}  else if (value*127 == 3) {
                ...
                case 76: { 
                    ...
                    if (value == 0) { 
                        ...
                        p5.viewportconf.cammode = "fullscreen"
                    } 
                    break;
                }
                case 77: {p5.viewportconf.audioCutoff = value*127, a.setCutoff(p5.viewportconf.audioCutoff); break; }
                case 78: {p5.viewportconf.audioScale = value*127, a.setScale(p5.viewportconf.audioScale); break; }  
        ...
        if (cc.channel  == 6) { // img
            switch (index) {
                case 1: {
					console.log("setting image stuff: ", value)
					if (cc.value == 0){
						p5.imgconfig.render = false
					} else if (cc.value == 127){
						p5.imgconfig.render = true
					}
                    if (cc.value == 1){ 
						randomImage(p5)
						p5.imgconfig.mode = "cube"
						console.log(p5.imgconfig)
					} else if (cc.value == 2){  
						randomImage(p5)
						p5.imgconfig.mode = "center"
					} break;
                }
            }
        }
		if (cc.channel == 7 ) { // video
			switch (index) {
                case 1: {
					console.log("setting enabling video: ", value)
					if (cc.value == 0){
						p5.vidconfig.render = false
					} else if (cc.value == 127){
				...
				case 2: {
 					if (cc.value == 1){ 
						...
						// randomVideo(p5)
						p5.vidconfig.mode = "center"
					} else if (cc.value == 3) {
						p5.vidconfig.mode = "fullscreen"
					} break;
				} 
				case 3:{
					viddataset = viddatasetlist[cc.value]
					break;
                }
			}
		}
